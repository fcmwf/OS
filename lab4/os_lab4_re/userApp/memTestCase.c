...
int testCase1(int argc, unsigned char **argv){  
	//======for malloc===============================
	char*buf1 = (char*)malloc_1(19);
	char*buf2 = (char*)malloc_1(24);
...
int testCase2(int argc, unsigned char **argv){
	//======for malloc===============================
	char*buf1 = (char*)malloc_1(11);
	char*buf2 = (char*)malloc_1(21);
...
int maxMallocSizeNow(int argc, unsigned char **argv){
	...
	while(1){
		...
		i += 0x7E00000;   //个人调大了一些，要不然太慢了
		//myPrintf(0x7,"i=%x\n",i);
	}
	myPrintf(0x7, "MAX_MALLOC_SIZE: 0x%x (with step = 0x7E00000);\n",i);	
}
...
int testeFP(int argc, unsigned char **argv){
	...
	unsigned long x,x1,x2,x3,x4,x5;
	volatile unsigned long xHandler;
	...
	x = malloc_1(tsize); myPrintf(0x7,"X:0x%x:%d \n",x,tsize);
	if (x) {
		myPrintf(0x7, "We had successfully ");
		myPrintf(0x5, "malloc()");
		myPrintf(0x7, " a small memBlock (size=0x%x, addr=0x%x);\n", tsize,x);
		...
		unsigned long x6,x7,x8,x9;
		myPrintf(0x7,"Now, release A.\n");
		eFPartitionFree(xHandler,x1);
		// if(x6) myPrintf(0x7,"free complete\n");
		// else   myPrintf(0x7,"free failed\n");
		eFPartitionWalkByAddr(xHandler);

		myPrintf(0x7,"Now, release B.\n");
		eFPartitionFree(xHandler,x2);
		// if(x7) myPrintf(0x7,"free complete\n");
		// else   myPrintf(0x7,"free failed\n");
		eFPartitionWalkByAddr(xHandler);

		myPrintf(0x7,"Now, release C.\n");
		eFPartitionFree(xHandler,x3);
		// if(x8) myPrintf(0x7,"free complete\n");
		// else   myPrintf(0x7,"free failed\n");
		eFPartitionWalkByAddr(xHandler);

		myPrintf(0x7,"Now, release D.\n");
		eFPartitionFree(xHandler,x4);	
		// if(x9) myPrintf(0x7,"free complete\n");
		// else   myPrintf(0x7,"free failed\n");	
		eFPartitionWalkByAddr(xHandler);
